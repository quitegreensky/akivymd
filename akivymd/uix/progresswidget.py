...
class AKCircularProgress(ThemableBehavior, BoxLayout):
    ...
    def on__current_deg(self, *args):
        if self.percent_type=='percent':
            self.ids._percent_label.text= str(int( (self._current_deg-self.start_deg) /self._each_percent))+ ' %'
        elif self.percent_type=='relative':
            self.ids._percent_label.text= str(int((self._current_deg-self.start_deg)/self._each_percent))+'\\'+ str(self.max_percent)
