...
class AKFloatingWindow(ThemableBehavior,RectangularElevationBehavior, BoxLayout):
    ...
    def _update_open_pos(self):
            self.open_position = [ Window.size[0]/2-self.width/2, Window.size[1]/2-self.height/2]
    ...
    def open(self):
        ...
        if self.fade_open:
            anim= Animation(pos=self.open_position, duration= 0)+\
                Animation(opacity=1, t= self.animation_transition, duration= self.animation_duration)
            anim.start(self)
        else:
            self.opacity= 1
    ...
    def minimize_to_normal(self):
        ...
        if self.minimize_animation: 
            anim= Animation(pos=pos, size=size, duration= self.animation_duration, t= self.animation_transition)
            anim.start(self)
        else: 
            self.pos= pos 
            self.size= size
...
class AKFloatingWindowLayout(ThemableBehavior, FloatLayout):
    ...
    def on_touch_down(self, touch):
        ...
        if clicked_window==top_window and header.collide_point(touch_pos[0], touch_pos[1]):
