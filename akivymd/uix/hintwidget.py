...
class AKHintWidget(BoxLayout):
    ...
    def _change_opacity(self, opacity):
        if not self._opac_anim_started:
            anim= Animation(_hintwidget_alpha=opacity, duration= self.opacity_duration, t= self.transition)
            anim.start(self)
            self._opac_anim_started= True 
            Clock.schedule_once(lambda x: self._allow_opac_animation(), self.opacity_duration)
    ...
    def on_touch_down(self, touch):
        ...
        if self.show_mode=='touch' and self.collide_point(pos[0], pos[1]):
            ...
            if self._state=='open' and not self.ids._float_box.collide_point(pos[0],pos[1]):
                opac= 0
                self._state='close'
    ...
    def on_touch_up(self, touch):
        ...
        if self.show_mode=='touch':
            if not self.auto_dismiss and self._state=='open':
                ...
                opac=0
                self._state='close'
            Clock.schedule_once(lambda x:self._change_opacity(opac) , self.opacity_duration)
